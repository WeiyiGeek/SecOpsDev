#!/bin/bash
# @Author: WeiyiGeek
# @Description: Ubuntu 20.04 TLS Security Initiate
...
#-------------------------------------------------#
# 脚本主要功能说明:
# (1) Ubuntu 20.04系统初始化操作包括IP地址设置、基础软件包更新以及安装加固。
# (2) Ubuntu 20.04系统容器以及JDK相关环境安装。
# (3) Ubuntu 20.04系统中异常错误日志解决。
#-------------------------------------------------#

# 系统全局变量定义
NAME=SecurityServerTemplate
IP=192.168.1.2
GATEWAY=192.168.1.1
SSHPORT=20211
...
## 参数: 无
os::network () {
log::info "正在进行->网络配置相关操作脚本包括(IP地址修改)"
...
sudo /opt/init/network.sh ${IP} ${GATEWAY} ${NAME}
...
## 名称: os::security
## 用途: 操作系统安全加固配置(符合等保要求-三级要求)
## 参数: 无
os::security () {
log::info "正在进行->操作系统安全加固(符合等保要求-三级要求)配置"

# (0) 系统用户核查配置
echo \*\*\*\* 锁定或者删除多余的系统账户以及创建低权限用户 \*\*\*\* 
userdel -r lxd
groupdel lxd
...
echo \*\*\*\* 锁定或删除帐号完成  \*\*\*\* 
echo \*\*\*\* 获取当前系统中的用户并警告异常用户 \*\*\*\* 
default=(root daemon bin sys games man lp mail news uucp proxy www-data backup list irc gnats nobody systemd-network systemd-resolve systemd-timesync messagebus syslog _apt tss uuidd tcpdump landscape pollinate usbmux sshd systemd-coredump _chrony)
...
egrep -q "^\s*PASS_MIN_LEN\s+\S*(\s*#.*)?\s*$" /etc/login.defs && sed -ri "s/^(\s*)PASS_MIN_LEN\s+\S*(\s*#.*)?\s*$/\PASS_MIN_LEN  12/" /etc/login.defs || echo "PASS_MIN_LEN  15" >> /etc/login.defs
...
egrep -q "^password\s.+pam_unix.so\s+\w+.*$" /etc/pam.d/common-password && sed -ri '/^password\s.+pam_unix.so/{s/pam_unix.so\s+\w+.*$/pam_cracklib.so obscure use_authtok try_first_pass sha512 remember=3/g;}' /etc/pam.d/common-password
touch /etc/security/opasswd &&  chown root:root /etc/security/opasswd && chmod 600 /etc/security/opasswd  # 存储用户密码经过sha512加密所以非常注意其权限



# (3) 用户sudo权限以及重要目录和文件的权限设置
echo \*\*\*\* 默认用户的sudo权限限制  \*\*\*\* 
# Sudo允许授权用户权限以另一个用户（通常是root用户）的身份运行程序, 如Ubuntu安装时您创建的用户 WeiyiGeek 防止直接通过 sudo passwd 修改root密码(此时必须要求输入WeiyiGeek密码后才可修改root密码)
sed -i "/# Members of the admin/i ${DefaultUser} ALL=(ALL) PASSWD:ALL" /etc/sudoers


echo \*\*\*\* 用户的umask安全配置修改为022  \*\*\*\* 
...
# (5) 用户远程登录失败次数与终端超时设置 （）
echo \*\*\*\* 远程连续登录失败5次锁定帐号5分钟包括root账号 \*\*\*\*
...
mv /usr/bin/su /usr/bin/switch_user
...
echo \*\*\*\* 系统 GRUB 安全设置(防止物理接触从grub菜单中修改密码) \*\*\*\*
# 设置Grub菜单界面显示时间
cp -a /etc/grub.d/00_header /tmp/
...
## 参数: 无
os::operation () {
log::info "正在进行->操作系统安全运维配置"

# (1) 设置文件删除回收站别名
echo \*\*\*\* 设置文件删除回收站别名(防止误删文件) \*\*\*\*
...
EOF
sudo chmod +775 /usr/local/bin/remove.sh /etc/profile.d/alias.sh /etc/profile.d/history-record.sh /usr/local/bin/history.sh 
sudo chmod a+x /usr/local/bin/remove.sh /etc/profile.d/alias.sh /etc/profile.d/history-record.sh /usr/local/bin/history.sh 
source /etc/profile.d/alias.sh  /etc/profile.d/history-record.sh

# (2) 解决普通定时任务无法后台定时执行
echo \*\*\*\* 解决普通定时任务无法后台定时执行 \*\*\*\*
...
## 参数: 无
os::optimizationn () {
log::info "正在进行->操作系统优化设置(内核参数)"

# (1) 系统内核参数的配置(/etc/sysctl.conf)
echo \*\*\*\* 系统内核参数的配置(/etc/sysctl.conf) \*\*\*\*
