...
function net_config () 
{
  ...
  if [ -f /etc/netplan/00-installer-config.yaml ];then
    cp /etc/netplan/00-installer-config.yaml ${BACKUPDIR}
    if [ ! -f /opt/init/ ];then mkdir -vp /opt/init/;fi
sudo tee /opt/init/network.sh <<'EOF'
#!/bin/bash
# @Author: WeiyiGeek
# @Description: Configure Ubuntu 20.04 、 22.04 Server Network
...
EOF
  ...
  log::info "VAR_IP = ${VAR_IP} , VAR_GATEWAY = ${VAR_GATEWAY}"
    ...
    log::error "Not found /etc/netplan/00-installer-config.yaml file."
  fi
