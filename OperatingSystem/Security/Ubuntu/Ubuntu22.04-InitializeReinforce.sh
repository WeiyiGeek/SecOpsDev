#!/bin/bash
# @Author: WeiyiGeek
# @Description: Ubuntu 22.04 Security Reinforce and System initialization
# @Create Time:  2022年8月18日 14:39:06
# @Last Modified time: 2022年8月18日 14:39:09
...
start::check () {
  ...
  if [ $? != 0 ]; then
    printf '%s\n' "$(tput setaf 1)Error: script is only available for Ubuntu systems.$(tput sgr0)" >&2
    exit 1
  fi
...
start::banner (){
  ...
  printf "\033[32m@ Desc: Ubuntu 22.04 Security Reinforce and System initialization \033[0m\n"
...
start::help ()
{
  ...
  log::warning "温馨提示：使用前先请配置机器上网环境,若没有配置请在 Ubuntu22.04.conf 文件中进行网络配置."
  exit 0
}
...
main () {
  ...
  # Load Configure File.
  source config/Ubuntu22.04.conf
